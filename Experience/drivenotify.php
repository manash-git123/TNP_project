...
function mailer($name,$email){
  ...
  $mail->Username   = "official gmail";
  $mail->Password   = "gmail password";

  $mail->IsHTML(true);
  $mail->AddAddress($email, $name);
  $mail->SetFrom("official gmail", "NIT-TNP-CELL");
//   $mail->AddReplyTo("reply-to-email", "reply-to-name");
//   $mail->AddCC("cc-recipient-email", "cc-recipient-name");
  $mail->Subject = "New drive";
  $content = "Drive details vorabi";
...
if($row20['BTechEligibility']==1 && $B_cse==1){
     ...
     $num = mysqli_num_rows($result10);
     $sqlQuery = "SELECT * FROM interneligibility WHERE ID = ".$driveid."";
     $resQuery = mysqli_query($con,$sqlQuery);
     $rowYear = mysqli_fetch_assoc($resQuery);
     for($i = 0 ; $i<$num ; $i++)
     { 
       $scid = $row10[$i][0];
       $year1 = (integer)(2000 +($scid/100000));
