...
<body style="padding-bottom: 10px; background-image: url(../images/newspaper.jpg); background-size: cover;">
	...
	<hr>
	<h2  style="text-align: center;">The Students didn't respond to the drive</h2>
	<div style="box-shadow: 5px 5px 5px 5px gray;"class="container">
		<div class="row">
			...
			if (isset($_POST['noresponse_list']) || $_SESSION['drive_id']) {
			 	$DriveID = $_SESSION['drive_id'];
			 	$sql = "SELECT DISTINCT * FROM driveresponses WHERE DriveID = $DriveID AND status = 0";
				 $result = mysqli_query($con,$sql);
				 
				 $sql10 = "SELECT DISTINCT COUNT(DriveID) AS total FROM driveresponses WHERE DriveID = $DriveID AND status = 0";
        ...
        echo "<script>window.location.href = '../student_coo.php';</script>";				
				...
				echo "<h2>Drive ID :".$DriveID."</h2>";
			 	...
			 	<?php
				 $i=1;
				 $sql = "SELECT DISTINCT * FROM driveresponses WHERE DriveID = $DriveID AND status = 0 LIMIT " .$start_from. ',' .$results_per_page;
                 $result = mysqli_query($con,$sql);
			 ...
			 <tr style="background-color: white;">
			 	...
			 	<td><form action="../guest_view.php" method="post"><button class="button" style="padding-top:3px;padding-bottom:3px;"  name="scholarID" value="<?php echo $row['ScholarID']; ?>">Visit</button></form></td>
			 	<?php 
			...
			echo " </tbody>
                    ...
                    <button type='submit' style='background-color: #021b27;border-color: #021b27;' name='create_excel' value=0 id='create_excel' class='btn btn-success'>Create Excel File</button></form><br>";
			 }else{
...
<?php
		...
		{
			echo "<a style='font-size: 20px;background-color:rgb(44, 43, 43);color: white;border-radius:20%;' href='noresponse_list.php?page=".$page."'";
