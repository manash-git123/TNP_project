...
<body style="padding-bottom: 10px; background-image: url(../images/newspaper.jpg); background-size: cover;">
	...
	<hr>
	<h2 style="text-align: center;">The Students who rejected the drive</h2>
	<div style="border-radius: 10px;box-shadow: 5px 5px 5px 5px gray;" class="container">
		<div class="row">
			...
			if (isset($_POST['rejected_list']) || $_SESSION['drive_id']) {
			 	$DriveID = $_SESSION['drive_id'];
			 	$sql = "SELECT DISTINCT * FROM driveresponses WHERE DriveID = $DriveID AND status = 2";
				 $result = mysqli_query($con,$sql);
				 
				 $sql10 = "SELECT DISTINCT  COUNT(DriveID) AS total FROM driveresponses WHERE DriveID = $DriveID AND status = 2";
        ...
        echo "<script>window.location.href = '../student_coo.php';</script>";				
				...
				echo "<h2>Drive ID :".$DriveID."</h2>";
			 	...
			 	<?php
				 $i=1;
				 $sql = "SELECT DISTINCT * FROM driveresponses WHERE DriveID = $DriveID AND status = 2 LIMIT " .$start_from. ',' .$results_per_page;
                 $result = mysqli_query($con,$sql);
			...
			echo " </tbody>
                    ...
                    <button type='submit' style='background-color: #021b27;border-color: #021b27;' name='create_excel' value=2 id='create_excel' class='btn btn-success'>Create Excel File</button></form><br>";
			 }else{
		...
		{
			echo "<a style='font-size: 20px;background-color:rgb(44, 43, 43);color: white;border-radius:20%;' href='rejected_list.php?page=".$page."'";
